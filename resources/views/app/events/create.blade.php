...
<div class="py-5 text-center">
    <h4 class="text-secondary text-capitalize">create new event</h4>
</div>

<form class="mb-5" action="{{ route('events.store') }}" method="post">
    @csrf
            ...
            <select class="form-select" name="venue" id="venue-input"
                ...
                @foreach($venues as $venue)
                    <option value="{{ $venue->id }}"
                        {{ (old('venue') == $venue->id) ? 'selected="selected"' : '' }}>
                        {{ $venue->name }}
                    </option>
                @endforeach
            </select>
...
<div class="modal fade" id="addVenueModal" tabindex="-1" aria-labelledby="addVenueModalLabel" ` aria-hidden="true">
    ...
    <script>
        const baseString = 'https://www.google.com/maps/search/';
        var searchAddress = document.getElementById("googleMapSearch");
        searchAddress.onclick = function() {
            var address = document.getElementById("address-input").value;
            var formatedAddress = address.replace(/\+/g,' ');
            searchAddress.href = baseString + formatedAddress;
            window.open(baseString + formatedAddress, "_blank");
        }
    </script>
</div>
